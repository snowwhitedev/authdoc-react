...
function StyledTreeItem(props) {
  ...
  return ( nodeExpanded ?
    <>
      <TreeItem
        nodeId={nodeId}
        collapseIcon={<><ArrowDropDownIcon /><FolderOpen /></>}
        expandIcon={<><ArrowRightIcon /><Folder /></>}
        endIcon={ data.has_children || data.category_fk !== 1 ? <Folder /> : <FileDocument />}
        label={
          <div className={classes.labelRoot}>
            <Typography variant="body2" className={classes.labelText}>
              {labelText}
            </Typography>
            {(!data.has_children || data.has_children === 0) && data.category_fk === 1 &&
              <>
                <span className="icon-wrapper" >
                  <InfoOutlined onClick={() => handleShowDocumentCard(data.authority_document_fk)} />
                </span>
                <Checkbox
                  color="primary"
                  size="small"
                  checked={data.selected}
                  onChange={(e) => handleSelectDocument(data, e.target.checked)}
